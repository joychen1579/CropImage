package org.joychen.sheetdialogios;
...
public class IosSheetDialog {
    ...
    public IosSheetDialog builder() {
        ...
        View view = LayoutInflater.from(context).inflate(
                R.layout.ios_action_sheet, null);
        ...
        for (int i = 1; i <= size; i++) {
            ...
            if (size == 1) {
                ...
                if (showTitle) {
                    if (i >= 1 && i < size) {
                        if (i == selected) {
                            textView.setBackgroundResource(R.drawable.actionsheet_middle_selected);
                        }else{
    ...
    public void setPosition() {
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                maxScrollView = lLayout_content.getMeasuredHeight() - sLayout_content.getHeight();
                int toscroll = (maxScrollView / sheetItemList.size()) * selected - txt_title.getHeight();
                sLayout_content.scrollTo(0, toscroll);
            }
        }, 200);
